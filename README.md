...
Run from the repo root:
    ...
    cd "C:\Users\jacks\OneDrive\Desktop\voynich-os"
...
For historical outputs, use audit reports, ledgers, mini READMEs, and the professional directory plan instead of expanding the root README into a command archive.

## Repository Layers

| Layer | Purpose | Main paths |
|---|---|---|
| Corpus and generated outputs | Holds source transcription files and generated folio/meaning/manuscript artifacts | `data/corpus/`, `data/folio_outputs/`, `data/meaning/`, `data/manuscript_v12_0/` |
| Existing engine surface | Existing processing engine and runtime scripts | `engine/` |
| RCC-N navigation | Makes the repo self-locating for humans and AI agents | `rcc/nexus/`, `docs/context/`, folder `README.md` files |
| Audit and validation | Runs repo audit, JSON normalization, RCC checks, and tests | `scripts/`, `scripts/rcc/`, `tests/` |
| Evidence and reports | Holds RCC reports, README audits, reorg plans, and audit state | `reports/`, `state/audits/` |
| Documentation shell | Records claim boundaries, reproducibility, AI context, and professional reorg policy | `docs/` |
| Future evidence surfaces | Future run artifacts, source cards, and visuals | `artifacts/`, `sources/`, `visuals/` |
...
| Surface | Purpose |
|---|---|
| `data/corpus/` | Transcription corpus inputs and folio text surfaces. |
| `data/folio_outputs/` | Generated folio graph/state JSON artifacts. |
| `data/meaning/` | Generated meaning/vector/cluster diagnostics. |
| `data/manuscript_v12_0/` | Manuscript-level synthesis state. |
| `data/ledger/` | Run/upstream ledger artifacts. |
| `scripts/voynich_repo_audit.py` | Corpus/output/hash audit. |
| `scripts/normalize_json_utf8.py` | JSON encoding hygiene. |
| `scripts/rcc/check_rcc_nexus.py` | RCC-N navigation validation. |
| `scripts/rcc/audit_readme_surface.py` | README and mini README discipline validation. |
...
| Data/output patch | `docs/REPRODUCIBILITY.md`, `data/README.md` | JSON hygiene + repo audit + tests |
...
| VOS-L-006 | README audit reported mojibake after RCC injection. | Prior README contained encoding drift. | Public README must prefer ASCII-safe syntax unless Unicode is intentionally audited. |
...
| VOS-L-008 | Public README still opened with overclaiming language after governance sections were appended. | Old README content remained above the professional Nexus mirror. | Replace the public README opening with evidence-bounded framing before publishing. |
...
| Warning-inspection rule | Passing validators with warnings must be inspected and either repaired or classified as non-blocking. |
...
| Boundary rule | Process alignment is repository hygiene, not decipherment or translation proof. |
